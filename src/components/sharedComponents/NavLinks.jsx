...
export default function NavLinks({ rounded, border }) {
  ...
  return (
    <div
      ...
      className="flex w-full  transition-all duration-75 ease-in-out justify-around border-[#4D4D4D] bg-[#222222] py-[10px] px-[20px] items-center gap-[5px]  md:gap-[68px] md:text-[1.6rem] text-[1.4rem] font-[400] text-[white]"
    >
      {navsArray.map((item) => (
        <NavLink
          className={({ isActive }) =>
            `font-[400]     rounded-[48px]  transition-all duration-[1s] ease-in-out ${
              isActive
                ? "lg:px-[3.4rem] py-[13px]  md:px-[4.8rem] px-[2.2rem] text-[black] font-[700] bg-[#D7FD44]"
                : " py-[13px] px-[0.5rem]"
            }`
          }
          key={item.linkTo}
